...
describe Solidus::ProductsController, :type => :controller do
  ...
  it "allows admins to view non-active products" do
    allow(controller).to receive_messages :solidus_current_user => mock_model(Solidus.user_class, :has_solidus_role? => true, :last_incomplete_solidus_order => nil, :solidus_api_key => 'fake')
    solidus_get :show, :id => product.to_param
  ...
  it "doesn't error when given an invalid referer" do
    current_user = mock_model(Solidus.user_class, :has_solidus_role? => true, :last_incomplete_solidus_order => nil, :generate_solidus_api_key! => nil)
    allow(controller).to receive_messages :solidus_current_user => current_user
