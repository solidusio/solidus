...
module Spree
  describe OrdersController, :type => :controller do
    ...
    it 'should understand order routes with token' do
      expect(solidus.token_order_path('R123456', 'ABCDEF')).to eq('/orders/R123456/token/ABCDEF')
    end
    ...
    context 'when an order exists in the cookies.signed' do
      ...
      context '#edit' do
        it 'should check if user is authorized for :edit' do
          expect(controller).to receive(:authorize!).with(:edit, order, token)
          solidus_get :edit, :token => token
        end
        it "should check against the specified order" do
          expect(controller).to receive(:authorize!).with(:edit, specified_order, token)
          solidus_get :edit, :id => specified_order.number, :token => token
        end
      end
      ...
      context '#update' do
        it 'should check if user is authorized for :edit' do
          ...
          expect(controller).to receive(:authorize!).with(:edit, order, token)
          solidus_post :update, :order => { :email => "foo@bar.com" }, :token => token
        end
        it "should check against the specified order" do
          allow(order).to receive :update_attributes
          expect(controller).to receive(:authorize!).with(:edit, specified_order, token)
          solidus_post :update, :order => { :email => "foo@bar.com" }, :id => specified_order.number, :token => token
        end
      end
      ...
      context '#empty' do
        it 'should check if user is authorized for :edit' do
          expect(controller).to receive(:authorize!).with(:edit, order, token)
          solidus_post :empty, :token => token
        end
        it "should check against the specified order" do
          expect(controller).to receive(:authorize!).with(:edit, specified_order, token)
          solidus_post :empty, :id => specified_order.number, :token => token
        end
      end
      ...
      context "#show" do
        it "should check against the specified order" do
          expect(controller).to receive(:authorize!).with(:edit, specified_order, token)
          solidus_get :show, :id => specified_order.number, :token => token
        end
      end
    end
      ...
      context '#show' do
        context 'when token parameter present' do
          it 'always ooverride existing token when passing a new one' do
            ...
            solidus_get :show, { :id => 'R123', :token => order.guest_token }
