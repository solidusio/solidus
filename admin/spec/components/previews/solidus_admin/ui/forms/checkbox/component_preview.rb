...
class SolidusAdmin::UI::Forms::Checkbox::ComponentPreview < ViewComponent::Preview
  ...
  # @param size [Symbol] [:s, :m]
  # @param checked toggle
  # @param disabled toggle
  # @param indeterminate toggle
  # @param attributes text Additional attributes in JSON format
  def playground(size: :m, checked: false, indeterminate: false, disabled: false, attributes: nil)
    attributes = JSON.parse(attributes) if attributes
    attributes ||= {}
    attributes[:onload] = "this.indeterminate = true" if indeterminate
    render current_component.new(size: size, checked: checked, disabled: disabled, **attributes.to_h)
  end
end
