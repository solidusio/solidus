...
module Spree
  describe Spree::Api::PropertiesController, type: :request do
    ...
    it 'can query the results through a parameter' do
      get :index, params: { q: { name_cont: 'ba' } }
    ...
    it "retrieves a list of properties by id" do
      get :index, params: { ids: [property_1.id] }
      expect(json_response["properties"].first).to have_attributes(attributes)
      expect(json_response["count"]).to eq(1)
    end

    it "retrieves a list of properties by ids string" do
      get :index, params: { ids: [property_1.id, property_2.id].join(",") }
      expect(json_response["properties"].first).to have_attributes(attributes)
    ...
    it "can see a property by name" do
      get :show, params: { id: property_1.name }
      expect(json_response).to have_attributes(attributes)
    end
    ...
    it "cannot create a new property if not an admin" do
      post :create, params: { property: { name: "My Property 3" } }
      assert_unauthorized!
    end

    it "cannot update a property" do
      put :update, params: { id: property_1.name, property: { presentation: "my value 456" } }
      assert_unauthorized!
    end

    it "cannot delete a property" do
      delete :destroy, params: { id: property_1.id }
      assert_unauthorized!
    ...
    context "as an admin" do
      ...
      it "can create a new property" do
        ...
        post :create, params: { property: { name: "My Property 3", presentation: "my value 3" } }
        expect(json_response).to have_attributes(attributes)
      ...
      it "can delete a property" do
        delete :destroy, params: { id: property_1.name }
