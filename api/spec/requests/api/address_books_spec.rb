    ...
    context 'as address book owner' do
      context 'with ability' do
        ...
        it 'updates my address book' do
          ...
          user = create(:user, spree_api_key: 'galleon')
          address = user.save_in_address_book(address_attributes, true)
          address_attributes['firstname'] = 'Ron'

          expect {
            put "/api/users/#{user.id}/address_book", { address_book: address_attributes.merge('id' => address.id) }, { 'X-SPREE-TOKEN' => 'galleon' }
          }.to change { UserAddress.count }.from(1).to(2)

          expect(response.status).to eq(200)
          expect(JSON.parse(response.body).first).to include(address_attributes)
        end
