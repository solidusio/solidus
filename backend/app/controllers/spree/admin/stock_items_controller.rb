...
module Solidus
  module Admin
    class StockItemsController < ResourceController
      ...
      def build_resource
        variant = Solidus::Variant.accessible_by(current_ability, :read).find(params[:variant_id])
      ...
      def load_product
        @product = Solidus::Product.accessible_by(current_ability, :read).friendly.find(params[:product_slug]) if params[:product_slug]
      end
      ...
      def load_stock_management_data
        ...
        @variants = Solidus::Config.variant_search_class.new(params[:variant_search_term], scope: variant_scope).results
        ...
        @variants = @variants.order(id: :desc).page(params[:page]).per(params[:per_page] || Solidus::Config[:orders_per_page])
      end

      def variant_scope
        scope = Solidus::Variant.accessible_by(current_ability, :read)
