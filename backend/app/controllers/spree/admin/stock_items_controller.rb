...
module Spree
  module Admin
    class StockItemsController < ResourceController
      ...
      def variant_scope
        scope = Spree::Variant.accessible_by(current_ability, :read).distinct.order(:sku).
            includes(:images,
                     stock_items: :stock_location,
                     product: :variant_images,
                     option_values: :option_type)
        scope.where!(product: @product) if @product
        scope.by_stock_location(params[:stock_location_id].presence)
      end
