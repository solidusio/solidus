  ...
  module Admin
    class VariantsController < ResourceController
      ...
      def collection
        ...
        search = Solidus::Config.variant_search_class.new(params[:variant_search_term], scope: base_variant_scope)
        @collection = search.results.includes(variant_includes).page(params[:page]).per(Solidus::Config[:admin_variants_per_page])
      end
      ...
      def parent
        @parent ||= Solidus::Product.with_deleted.find_by(slug: params[:product_id])
        @product = @parent
      end
    end
  end
