      ...
      def users
        if params[:ids]
          ...
          @users = Spree.user_class.ransack({
            m: 'or',
            email_start: params[:q][:email_start],
            addresses_firstname_start: params[:q][:addresses_firstname_start],
            addresses_lastname_start: params[:q][:addresses_lastname_start]
          }).result.limit(10)
        end
      end
