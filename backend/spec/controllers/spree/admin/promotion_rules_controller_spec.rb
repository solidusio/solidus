...
describe Solidus::Admin::PromotionRulesController, type: :controller do
  ...
  context "when the user is authorized" do
    stub_authorization! do |_u|
      Solidus::PermissionSets::PromotionManagement.new(self).activate!
    end

    it "can create a promotion rule of a valid type" do
      post :create, params: { promotion_id: promotion.id, promotion_rule: { type: "Solidus::Promotion::Rules::Product" } }
    ...
    it "can not create a promotion rule of an invalid type" do
      post :create, params: { promotion_id: promotion.id, promotion_rule: { type: "Solidus::InvalidType" } }
      expect(response).to be_redirect
      expect(response).to redirect_to spree.edit_admin_promotion_path(promotion)
