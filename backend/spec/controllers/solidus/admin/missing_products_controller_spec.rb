...
# the load_resource filter in Solidus::Admin::ResourceController is prepended to the filter chain
...
describe Solidus::Admin::ProductsController, :type => :controller do
  ...
  it "cannot find a non-existent product" do
    solidus_get :edit, :id => "non-existent-product"
    expect(response).to redirect_to(solidus.admin_products_path)
