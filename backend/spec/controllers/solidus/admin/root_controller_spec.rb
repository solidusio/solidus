...
describe Solidus::Admin::RootController do
  describe "GET index" do
    ...
    before do
      allow_any_instance_of(Solidus::Admin::RootController).to receive(:try_solidus_current_user).and_return(user)
      allow_any_instance_of(Solidus::Admin::RootController).to receive(:current_ability).and_return(ability)
    end

    context "when a user can admin and display solidus orders" do
      before do
        ability.can :admin, Solidus::Order
        ability.can :display, Solidus::Order
      end

      it { is_expected.to redirect_to(solidus.admin_orders_path) }
    end

    context "when a user cannot admin and display solidus orders" do
      context "when a user can admin and home dashboards" do
        ...
        it { is_expected.to redirect_to(solidus.home_admin_dashboards_path) }
      end
