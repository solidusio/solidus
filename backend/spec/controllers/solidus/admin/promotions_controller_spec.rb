...
describe Solidus::Admin::PromotionsController, :type => :controller do
  ...
  describe "#index" do
    ...
    context "search" do
      it "pages results" do
        solidus_get :index, per_page: '1'
        expect(assigns[:promotions]).to eq [promotion2]
      end

      it "filters by name" do
        solidus_get :index, q: {name_cont: promotion1.name}
        expect(assigns[:promotions]).to eq [promotion1]
      end

      it "filters by code" do
        solidus_get :index, q: {codes_value_cont: promotion1.codes.first.value }
        expect(assigns[:promotions]).to eq [promotion1]
      end

      it "filters by path" do
        solidus_get :index, q: {path_cont: promotion1.path}
        expect(assigns[:promotions]).to eq [promotion1]
      end
    end
  end
