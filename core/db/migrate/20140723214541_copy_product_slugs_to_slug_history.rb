class CopyProductSlugsToSlugHistory < ActiveRecord::Migration
  def change
	...
	Solidus::Product.connection.execute <<-SQL
INSERT INTO #{FriendlyId::Slug.table_name} (slug, sluggable_id, sluggable_type, created_at)
  SELECT slug, id, '#{Solidus::Product.to_s}', #{ActiveRecord::Base.send(:sanitize_sql_array, ['?', Time.current])} 
  FROM #{Solidus::Product.table_name}
