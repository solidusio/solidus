class SetShipmentTotalForUsersUpgrading < ActiveRecord::Migration
  def up
    # NOTE You might not need this at all unless you're upgrading from Solidus 2.1.x
    # or below. For those upgrading this should populate the Order#shipment_total
    # for legacy orders
    Solidus::Order.complete.where('shipment_total = ?', 0).includes(:shipments).find_each do |order|
