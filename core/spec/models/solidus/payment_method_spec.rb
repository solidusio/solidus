...
RSpec.describe Solidus::PaymentMethod, type: :model do
  ...
  describe "available_to_[<users>, <admin>, <store>]" do
    context "when searching for payment methods available to users and admins" do
      subject { Solidus::PaymentMethod.available_to_users.available_to_admin }
      ...
      context "with a store" do
        ...
        subject { Solidus::PaymentMethod.available_to_store( store_1 ).available_to_users.available_to_admin }

        it { is_expected.to contain_exactly(payment_method_both_display, payment_method_nil_display) }

        context "when the store has no payment methods" do
          subject { Solidus::PaymentMethod.available_to_store(store_without_payment_methods) }
    ...
    context "when searching for payment methods available to users" do
      subject { Solidus::PaymentMethod.available_to_users }
    ...
    context "when searching for payment methods available to admin" do
      subject { Solidus::PaymentMethod.available_to_admin }
    ...
    context "when searching for payment methods available to a store" do
      subject { Solidus::PaymentMethod.available_to_store(store) }
  ...
  describe ".available" do
    it "should have 4 total methods" do
      expect(Solidus::PaymentMethod.all.size).to eq(4)
    end
      ...
      context 'when the store is specified' do
        context 'when the store has payment methods' do
          it 'finds the payment methods for the store' do
            Solidus::Deprecation.silence do
              expect(Solidus::PaymentMethod.available(:both, store: store_1)).to match_array(
        ...
        context "when store does not have payment_methods" do
          it "returns all matching payment methods regardless of store" do
            Solidus::Deprecation.silence do
              expect(Solidus::PaymentMethod.available(:both)).to match_array(
                [
                  payment_method_nil_display,
                  payment_method_both_display,
  ...
  describe '#auto_capture?' do
    class TestGateway < Solidus::PaymentMethod::CreditCard
      def gateway_class
        Provider
      end
    end
    ...
    context 'when auto_capture is nil' do
      before(:each) do
        expect(Solidus::Config).to receive('[]').with(:auto_capture).and_return(auto_capture)
      end

      context 'and when Solidus::Config[:auto_capture] is false' do
      ...
      context 'and when Solidus::Config[:auto_capture] is true' do
  ...
  describe 'ActiveMerchant methods' do
    ...
    class TestPaymentMethod < Solidus::PaymentMethod
      def gateway_class
        PaymentGateway
      end
    end
  ...
  describe 'model_name.human' do
    context 'PaymentMethod itself' do
      it "returns i18n value" do
        expect(Solidus::PaymentMethod.model_name.human).to eq('Payment Method')
      end
    end
  ...
  describe "::DISPLAY" do
    it "returns [:both, :front_end, :back_end]" do
      ...
      Solidus::Deprecation.silence do
        expect(Solidus::PaymentMethod::DISPLAY).to eq([:both, :front_end, :back_end])
      end
