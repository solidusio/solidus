...
describe Solidus::Shipment, :type => :model do
  ...
  let(:line_item) { mock_model(Solidus::LineItem, variant: variant) }
  ...
  context '#determine_state' do
    ...
    it 'returns pending if backordered' do
      allow(shipment).to receive_messages inventory_units: [mock_model(Solidus::InventoryUnit, backordered?: true)]
      expect(shipment.determine_state(order)).to eq 'pending'
    end
  ...
  context "display_amount" do
    it "retuns a Solidus::Money" do
      shipment.cost = 21.22
      expect(shipment.display_amount).to eq(Solidus::Money.new(21.22))
    end
  end
  ...
  context "display_final_price" do
    it "retuns a Solidus::Money" do
      ...
      expect(shipment.display_final_price).to eq(Solidus::Money.new(21.22))
    end
  end
  ...
  context "display_item_cost" do
    it "retuns a Solidus::Money" do
      ...
      expect(shipment.display_item_cost).to eq(Solidus::Money.new(21.22))
    end
  end
  ...
  context 'shipping_rates' do
    ...
    let(:shipping_rates) { [
      Solidus::ShippingRate.new(shipping_method: shipping_method1, cost: 10.00, selected: true),
      Solidus::ShippingRate.new(shipping_method: shipping_method2, cost: 20.00)
    ] }
    ...
    context 'refresh_rates' do
      ...
      it 'should request new rates, and maintain shipping_method selection' do
        expect(Solidus::Stock::Estimator).to receive(:new).with(shipment.order).and_return(mock_estimator)
      ...
      it 'should not refresh if shipment is shipped' do
        expect(Solidus::Stock::Estimator).not_to receive(:new)
        shipment.shipping_rates.delete_all
  ...
  context "#update!" do
    ...
    context "when payment is not required" do
      before do
        @original_require_payment = Solidus::Config[:require_payment_to_ship]
        Solidus::Config[:require_payment_to_ship] = false
      end

      after do
        Solidus::Config[:require_payment_to_ship] = @original_require_payment
      end
    ...
    context "when shipment state changes to shipped" do
      ...
      context "with adjustments" do
        before do
          shipment.adjustments << Solidus::Adjustment.create(order: order, label: "Label", amount: 5)
        end
  ...
  context "when order is completed" do
    ...
    context "with inventory tracking" do
      before { Solidus::Config.set track_inventory_levels: true }
    ...
    context "without inventory tracking" do
      before { Solidus::Config.set track_inventory_levels: false }
  ...
  context "#cancel" do
    ...
    it 'restocks the items' do
      ...
      shipment.stock_location = mock_model(Solidus::StockLocation)
  ...
  context "#resume" do
    ...
    context "when any inventory is backordered" do
      before { allow_any_instance_of(Solidus::InventoryUnit).to receive(:backordered?).and_return(true) }
      it "should result in a 'ready' state" do
        shipment.resume!
        expect(shipment.state).to eq 'pending'
      end
    ...
    context "when the order is paid, shippable, and not backordered" do
      before do
        ...
        allow(order).to receive_messages(paid?: true)
        allow_any_instance_of(Solidus::InventoryUnit).to receive(:backordered?).and_return(false)
      end
  ...
  context "#ship" do
    context "when the shipment is canceled" do
      ...
      it 'unstocks them items' do
        expect(shipment_with_inventory_units.stock_location).to receive(:unstock).with(an_instance_of(Solidus::Variant), 1, shipment_with_inventory_units)
        subject
      end
    end
    ...
    ['ready', 'canceled'].each do |state|
      context "from #{state}" do
        ...
        it "should call fulfill_order_with_stock_location" do
          expect(Solidus::OrderStockLocation).to(
  ...
  context "changes shipping rate via general update" do
    ...
    let(:shipment) { Solidus::Shipment.create order_id: order.id }

    let(:shipping_rate) do
      Solidus::ShippingRate.create shipment_id: shipment.id, cost: 10
    end
  ...
  context "#destroy" do
    it "destroys linked shipping_rates" do
      reflection = Solidus::Shipment.reflect_on_association(:shipping_rates)
  ...
  describe ".by_store" do
    it "returns shipments by store" do
      ...
      shipments = Solidus::Shipment.by_store(olivanders_store)

      expect(Solidus::Shipment.count).to eq(2)
