...
describe Spree::Wallet, type: :model do
  ...
  describe "#find_by_payment_source" do
    context "with no payment sources in the wallet" do
      it "returns nil" do
        expect(subject.find_by_payment_source(1)).to be_nil
      end
    end
    ...
    context "with payment sources in the wallet" do
      ...
      it "finds the wallet payment source by payment source id" do
        expect(subject.find_by_payment_source(credit_card.id)).to eql wallet_credit_card
      end
    end
  end
