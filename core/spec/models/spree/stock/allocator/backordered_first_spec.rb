...
module Spree
  module Stock
    module Allocator
      RSpec.describe BackorderedFirst, type: :model do
        ...
        let!(:dropship_stock_location) { create(:stock_location) }
        ...
        describe '#allocate_inventory' do
          ...
          let(:on_hand_by_stock_location_id) do
            ...
            availability[dropship_stock_location.id] = Spree::StockQuantities.new(dropship_on_hand_availability)
            availability
          end
          ...
          let(:backorderable_by_stock_location_id) do
            availability = {}
            availability[dropship_stock_location.id] = Spree::StockQuantities.new(dropship_backorderable_availability)
            availability
          end
          ...
          context 'when the desired quantity for a variant is 0 and the variant is not present in a stock location' do
            ...
            let(:first_variant_desired) { 0 }
            let(:second_variant_desired) { 5 }

            before do
              availability.on_hand_by_stock_location_id.each_value do |stock_quantities|
                stock_quantities.quantities.delete(first_variant)
              end
            end

            it 'does not raise any errors' do
              expect { subject.allocate_inventory(desired) }.not_to raise_error
            end
          end
          ...
          context 'when default stock location has enough items' do
            ...
            it 'allocates all the desired units on the default stock location' do
              ...
              expect(on_hand_packages[dropship_stock_location.id][first_variant]).to eq(0)
              expect(on_hand_packages[dropship_stock_location.id][second_variant]).to eq(0)

              expect(backordered_packages[dropship_stock_location.id][first_variant]).to eq(0)
              expect(backordered_packages[dropship_stock_location.id][second_variant]).to eq(0)
          ...
          context 'when default stock location hasn\'t enough items' do
            ...
            context 'when dropship stock location has enough items' do
              ...
              it 'allocates all the desired units on the stock locations while stocks last' do
                ...
                expect(on_hand_packages[default_stock_location.id][first_variant]).to be_zero
                expect(on_hand_packages[default_stock_location.id][second_variant]).to eq(5)
                expect(on_hand_packages[dropship_stock_location.id][first_variant]).to eq(15)
                expect(on_hand_packages[dropship_stock_location.id][second_variant]).to eq(0)
            ...
            context 'when dropship stock location hasn\'t enough items' do
              ...
              it 'allocates all the desired units on the stock locations while stocks last' do
                ...
                expect(on_hand_packages[default_stock_location.id][first_variant]).to be_zero
                expect(on_hand_packages[default_stock_location.id][second_variant]).to eq(5)
                expect(on_hand_packages[dropship_stock_location.id][first_variant]).to eq(2)
                expect(on_hand_packages[dropship_stock_location.id][second_variant]).to eq(0)

                expect(backordered_packages[dropship_stock_location.id][first_variant]).to eq(13)
                expect(backordered_packages[dropship_stock_location.id][second_variant]).to eq(0)
