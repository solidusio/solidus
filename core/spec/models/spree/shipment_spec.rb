...
RSpec.describe Solidus::Shipment, type: :model do
  ...
  context '#transfer_to_location' do
    it 'transfers unit to a new shipment with given location' do
      ...
      aggregate_failures("verifying new shipment attributes") do
        ...
        end.to change { Solidus::Shipment.count }.by(1)
  ...
  context '#determine_state' do
    ...
    it 'returns pending if backordered' do
      allow(shipment).to receive_messages inventory_units: [mock_model(Solidus::InventoryUnit, allow_ship?: false, canceled?: false, shipped?: false)]
      expect(shipment.determine_state(order)).to eq 'pending'
    end
  ...
  context "display_total" do
    it "retuns a Solidus::Money" do
      ...
      expect(shipment.display_total).to eq(Solidus::Money.new(21.22))
    end
  end
  ...
  it "#discounted_cost" do
    ...
    expect(Solidus::Deprecation.silence { shipment.discounted_cost }).to eq(9)
  end
  ...
  context 'shipping_rates' do
    ...
    context 'refresh_rates' do
      ...
      it 'should request new rates, and maintain shipping_method selection' do
        expect(Solidus::Stock::Estimator).to receive(:new).with(no_args).and_return(mock_estimator)
  ...
  context "#resume" do
    ...
    context "when any inventory is backordered" do
      before { allow_any_instance_of(Solidus::InventoryUnit).to receive(:allow_ship?).and_return(false) }
      it "should result in a 'ready' state" do
        shipment.resume!
        expect(shipment.state).to eq 'pending'
      end
    ...
    context "when the order is paid, shippable, and not backordered" do
      before do
        ...
        allow(order).to receive_messages(paid?: true)
        allow_any_instance_of(Solidus::InventoryUnit).to receive(:allow_ship?).and_return(true)
      end
  ...
  context "#destroy" do
    let(:shipping_rate) do
      Solidus::ShippingRate.create!(
        shipping_method: shipping_method,
        selected: true,
        taxes: [Solidus::ShippingRateTax.new(amount: 20)]
      )
    end
  ...
  describe "#finalize!" do
    ...
    it "call run! on Solidus::Stock::InventoryUnitsFinalizer" do
    ...
    context "inventory unit already finalized" do
      ...
      it "doesn't pass the inventory unit" do
        expect(Solidus::Stock::InventoryUnitsFinalizer)
          .to receive(:new).with([])
