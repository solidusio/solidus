...
RSpec.describe Spree::Order, type: :model do
  ...
  describe "metadata fields" do
    it "responds to customer_metadata" do
      expect(order).to respond_to(:customer_metadata)
    end

    it "responds to admin_metadata" do
      expect(order).to respond_to(:admin_metadata)
    end

    it "can store data in customer_metadata" do
      order.customer_metadata = { "order_id" => "OD34236" }
      expect(order.customer_metadata["order_id"]).to eq("OD34236")
    end

    it "can store data in admin_metadata" do
      order.admin_metadata = { "internal_note" => "gift wrap the item" }
      expect(order.admin_metadata["internal_note"]).to eq("gift wrap the item")
    end
  end
