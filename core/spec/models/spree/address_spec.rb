...
RSpec.describe Spree::Address, type: :model do
  ...
  context 'name syncs with firstname and lastname' do
    ...
    it 'concatenates firstname and lastname' do
      expect(address.name).to eq('Ryan Bigg')
    end

    it 'updates name when updating firstname' do
      address.firstname = 'Brian'

      expect(address.name).to eq('Brian Bigg')
    end

    it 'updates name when updating lastname' do
      address.lastname = 'Small'
  ...
  context 'validation' do
    ...
    it 'requires name when firstname not present' do
      address.name = ''
      address.firstname = ''

      address.valid?

      expect(address.errors['name']).to eq(["can't be blank"])
    end

    it 'is valid when firstname is present' do
      address.name = ''
    ...
    context 'no user given' do
      ...
      context 'has a default country' do
        ...
        it 'sets up a new record with Spree::Config[:default_country_iso]' do
          expect(described_class.build_default.country).to eq default_country
        end
        ...
        it 'can override the country' do
          another_country = build_stubbed(:country)
          address = described_class.build_default(country: another_country)
      ...
      it 'raises ActiveRecord::RecordNotFound if :default_country_iso is set to an invalid value' do
        ...
        expect {
          described_class.build_default.country
  ...
  context '.immutable_merge' do
    ...
    subject { described_class.immutable_merge(existing_address, new_address_attributes) }
  ...
  describe '.value_attributes' do
    subject do
      described_class.value_attributes(base_attributes, merge_attributes)
    end
  ...
  describe '.taxation_attributes' do
    context 'both taxation and non-taxation attributes are present ' do
      let(:address) { described_class.new(name: 'Michael Jackson', state_id: 1, country_id: 2, zipcode: '12345') }
    ...
    context 'taxation attributes are blank' do
      let(:address) { described_class.new name: 'Michael Jackson' }
  ...
  context '#full_name' do
    context 'both first and last names are present' do
      let(:address) { described_class.new(firstname: 'Michael', lastname: 'Jackson') }
      specify { expect(address.full_name).to eq('Michael Jackson') }
    end

    context 'first name is blank' do
      let(:address) { described_class.new(firstname: nil, lastname: 'Jackson') }
      specify { expect(address.full_name).to eq('Jackson') }
    end

    context 'last name is blank' do
      let(:address) { described_class.new(firstname: 'Michael', lastname: nil) }
  ...
  context '#state_text' do
    context 'state is blank' do
      let(:address) { described_class.new(state: nil, state_name: 'virginia') }
      specify { expect(address.state_text).to eq('virginia') }
    end

    context 'both name and abbr is present' do
      let(:state) { Spree::State.new(name: 'virginia', abbr: 'va') }
      let(:address) { described_class.new(state: state) }
