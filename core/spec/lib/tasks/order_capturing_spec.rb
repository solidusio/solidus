...
describe "order_capturing:capture_payments" do
  ...
  context "with a mix of canceled and shipped inventory" do
    before do
      Solidus::OrderCancellations.new(order).short_ship([order.inventory_units.first])
    ...
    context "when there is an error capturing payment" do
      before do
        allow_any_instance_of(Solidus::OrderCapturing).to receive(:capture_payments).and_raise(StateMachines::InvalidTransition)
      end

      it "raises a OrderCapturingFailures" do
        expect { subject.invoke }.to raise_error(Solidus::OrderCapturingFailures)
      end
    end
  end
