...
RSpec.describe Spree::Core::NumberGenerator do
  ...
  describe 'model' do
    context "when class_name option is 'Spree::Shipment'" do
      subject do
        described_class.new(length: 3,
                            prefix: 'S',
                            class_name: 'Spree::Shipment').generate
      end

      it "Checks if the model exists" do
        expect(Spree::Shipment).to receive(:exists?).with(number: /\S{3}/)
        subject
      end
    end
  end
