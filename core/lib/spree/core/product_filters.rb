...
module Solidus
  module Core
    ...
    module ProductFilters
      ...
      #
      Solidus::Product.add_search_scope :price_range_any do |*opts|
        conds = opts.map { |o| Solidus::Core::ProductFilters.price_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Solidus::Product.joins(master: :default_price).where(scope)
      end

      def self.format_price(amount)
        Solidus::Money.new(amount)
      end
      ...
      Solidus::Product.add_search_scope :brand_any do |*opts|
        conds = opts.map { |o| ProductFilters.brand_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Solidus::Product.with_property('brand').where(scope)
      end

      def self.brand_filter
        brand_property = Solidus::Property.find_by(name: 'brand')
        brands = brand_property ? Solidus::ProductProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
        pp = Solidus::ProductProperty.arel_table
      ...
      Solidus::Product.add_search_scope :selective_brand_any do |*opts|
        Solidus::Product.brand_any(*opts)
      end

      def self.selective_brand_filter(taxon = nil)
        taxon ||= Solidus::Taxonomy.first.root
        brand_property = Solidus::Property.find_by(name: 'brand')
        scope = Solidus::ProductProperty.where(property: brand_property).
          joins(product: :taxons).
          where("#{Solidus::Taxon.table_name}.id" => [taxon] + taxon.descendants)
      ...
      def self.taxons_below(taxon)
        Solidus::Deprecation.warn "taxons_below is deprecated in solidus_core. Please add it to your own application to continue using it."
        return Solidus::Core::ProductFilters.all_taxons if taxon.nil?
        {
      ...
      def self.all_taxons
        Solidus::Deprecation.warn "all_taxons is deprecated in solidus_core. Please add it to your own application to continue using it."
        taxons = Solidus::Taxonomy.all.map { |t| [t.root] + t.root.descendants }.flatten
        {
          name:   'All taxons',
