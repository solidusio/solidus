...
module Solidus
  module Stock
    class Estimator
      ...
      #
      # @param package [Solidus::Stock::Package] the package to be shipped
      # @param frontend_only [Boolean] restricts the shipping methods to only
      #   those marked frontend if truthy
      # @return [Array<Solidus::ShippingRate>] the shipping rates sorted by
      ...
      def shipping_rates(package, frontend_only = true)
        ...
        Solidus::Config.shipping_rate_sorter_class.new(rates).sort
      end
      ...
      def choose_default_shipping_rate(shipping_rates)
        unless shipping_rates.empty?
          default_shipping_rate = Solidus::Config.shipping_rate_selector_class.new(shipping_rates).find_default
          default_shipping_rate.selected = true
        end
      end

      def calculate_shipping_rates(package)
        tax_calculator_class = Solidus::Config.shipping_rate_tax_calculator_class
