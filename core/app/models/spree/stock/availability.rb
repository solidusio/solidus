...
module Solidus
  module Stock
    ...
    #
    # This serves a similar role to Solidus::Stock::Quantifier, but is more
    # efficient by checking multiple variants at once.
    class Availability
      # @param variants [Array<Solidus::Variant>] variants to check stock of
      # @param stock_locations [Array<Solidus::StockLocation>] stock_locations to check for stock in
      def initialize(variants:, stock_locations: Solidus::StockLocation.active)
        @variants = variants
      ...
      # @return [Hash<Integer=>Solidus::StockQuantities>] A map of stock_location_ids to the stock quantities available in that location
      def on_hand_by_stock_location_id
        counts_on_hand.to_a.group_by do |(_, stock_location_id), _|
          stock_location_id
        end.transform_values do |values|
          Solidus::StockQuantities.new(
