    ...
    module Interactors
      class Performer
        ...
        delegate :reimbursement_tax_calculator, :reload, :calculated_total,
          ...
          :reimbursed!, :errored!, :errored?, :reimbursement_success_hooks,
          :send_reimbursement_email, :reimbursement_failure_hooks, :unpaid_amount,
          to: :reimbursement

        def call
          Spree::Event.instrument 'reimbursement_perform', reimbursement: reimbursement do
            unless created_by
            ...
            if unpaid_amount_within_tolerance?
              reimbursed!
              reimbursement_success_hooks.each { |h| h.call reimbursement } # TODO: these should become event subscriptions
            else
              errored!
              reimbursement_failure_hooks.each { |h| h.call reimbursement } # TODO: these should become event subscriptions
            end
          end
          if errored?
