...
# behaviour see Solidus::StoreSelector::Legacy.
#
# This attempts to find a Solidus::Store with a URL matching the domain name of
  ...
  module StoreSelector
    class ByServerName
      ...
      def store
        ...
        store = Solidus::Store.where(url: server_name).or(Store.where(default: true)).order(default: :asc).first

        # Provide a fallback, mostly for legacy/testing purposes
        store || Solidus::Store.new
      end
    end
  end
end
