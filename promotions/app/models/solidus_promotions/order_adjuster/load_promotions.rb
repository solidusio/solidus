  ...
  class OrderAdjuster
    class LoadPromotions
      ...
      def call
        ...
        promos.flat_map(&:benefits).group_by(&:preload_relations).each do |benefit_relations, benefits|
          preload(records: benefits, associations: benefit_relations)
          benefits.flat_map(&:conditions).group_by(&:preload_relations).each do |condition_relations, conditions|
            preload(records: conditions, associations: condition_relations)
          end
        end
