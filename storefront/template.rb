...
with_log = ->(message, &block) {
  say_status :installing, "[solidus_storefront] #{message}", :blue
  block.call
}
...
with_log["fetching remote templates"] do
  ...
  if __FILE__.match?(%r{\Ahttps?://})
    ...
    repo_dir = Rails.root.join("tmp/solidus_storefront-#{SecureRandom.hex}").tap(&:mkpath).to_s
...
with_log["installing files"] do
  ...
  # SolidusStorefront assets in CI.
