...
method = Solidus::PaymentMethod.where(:name => 'Credit Card', :active => true).first

# Hack the current method so we're able to return a gateway without a RAILS_ENV
Solidus::Gateway.class_eval do
  def self.current
    Solidus::Gateway::Bogus.new
  end
end
...
Solidus::Order.all.each_with_index do |order, index|
  order.update!
